...
namespace CPF_experiment
{
    ...
    public class WorldState : IComparable<IBinaryHeapItem> , IBinaryHeapItem
    {
        ...
        public Dictionary<int, List<double>> conflictProbability;
        /// <summary>
        ...
        public WorldState(AgentState[] allAgentsState, int minDepth = -1, int minCost = -1, int conflictRange = 0)
        {
            ...
            this.conflictProbability = new Dictionary<int, List<double>>();
        ...
        private int CompareToPRobust(WorldState other)
        {
            double probThis     = this.computeCollisionProbability();
            double probOther    = other.computeCollisionProbability();
            if (probThis < probOther)
                return 1;
            return -1;
        }

        private double computeCollisionProbability()
        {
            double result = 1;
            foreach(int key in conflictProbability.Keys)
            {
                List<double> currentList = conflictProbability[key];
                foreach(double probability in currentList)
                {
